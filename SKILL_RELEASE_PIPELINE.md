...
3. **امضای ثابت (بدون کامیت کلید):** امضای ریلیز فقط از طریق **Secrets مخفی گیت‌هاب**
   تأمین می‌شود (`KEYSTORE_BASE64`, `STORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`).
   فایل `release.keystore` هرگز در ریپو کامیت/میکانیت نمی‌شود؛ فینگرپرینت امضا در
   `SIGNING_FINGERPRINT.txt` هر Release درج و برای راستی‌آزمایی کاربر منتشر می‌گردد.
   کلید امضا باید خارج از ریپو (مخزن امن مالک پروژه) نگهداری و بکاپ شود.
...
```
## نصب بدون دردسر
APK ریلیس با امضای ثابت پروژه (کلید محرمانه‌ی خارج از ریپو) امضا شده؛ Play Protect ممکن است
برای اپ‌های خارج از استور اعلان عمومی بدهد (طبیعی و بی‌خطر) — فینگرپرینت SHA-256 در یادداشت
Release برای راستی‌آزمایی درج می‌شود.
