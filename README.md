...
## 📲 نصب اپلیکیشن اندروید (بدون نیاز به SDK / امضا / ابزار)

طبق Skill خط لوله انتشار (`SKILL_RELEASE_PIPELINE.md`)، هر نسخه پس از **پاس شدن ۱۰۰٪ تست ۱۰ سطحی**
به‌صورت خودکار در [Releases](https://github.com/IranCoin-premium/IBO.TradingSignals/releases) منتشر می‌شود:

1. به صفحه **Releases** بروید و آخرین نسخه (tag `vX.Y.Z`) را باز کنید.
2. فایل `IBO-TradingSignals-X.Y.Z-release.apk` را دانلود کنید.
3. نصب کنید — در صورت اعلان Play Protect برای اپ‌های خارج از استور، «Install anyway» طبیعی و بی‌خطر است.
4. (اختیاری) اصالت فایل: مقایسه SHA-256 با `SHA256SUMS.txt` و فینگرپرینت امضا در `SIGNING_FINGERPRINT.txt`.

- امضا **ثابت پروژه** است (کلید محرمانه خارج از ریپو، نه کامیت‌شده) → نسخه‌های جدید بدون حذف، روی نسخه قبلی نصب می‌شوند. فینگرپرینت امضا در هر Release منتشر می‌شود.
- APK دیباگ هم برای تست‌کنندگان در همان Release موجود است.
- دروازه انتشار: `bash scripts/verify-full-10level.sh` (الزام: همه‌ی ۱۰ سطح سبز) — یکپارچگی UI با `scripts/verify-ui-uniformity.sh` (قالب مرجع: صفحه Home / Soft-UI).
