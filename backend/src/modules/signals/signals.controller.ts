...
import { notificationOrchestrator } from '../mobile/notification-orchestrator.service';
...
export const createSignal = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Orchestrate Push Notifications (Part 28)
    try {
      await notificationOrchestrator.sendSignalAlert({
        id: newSignal.id,
        symbol: newSignal.asset,
        direction: newSignal.direction,
        strikePrice: newSignal.entry_price.toString(),
        duration: '5m'
      });
    } catch (pushError) {
      logger.error('Failed to dispatch push notification for new signal', pushError);
    }
