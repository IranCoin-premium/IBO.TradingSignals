...
export class PaymentsRepository {
  ...
  static async verifyAndActivateTransaction(
    ...
    try {
      ...
      // 8. Referral & Sales Partnership (Part 12): commission accrual INSIDE the same
      // DB transaction — payment success and commission are atomic. No attribution → no-op.
      try {
        await ReferralService.recordConversion({
          referredUserId: String(tx.user_id),
          orderId: null,
          transactionId: String(txId),
          planId: tx.plan_id ? String(tx.plan_id) : null,
          amount: String(tx.amount),
          currency: tx.currency || 'USD',
        });
      } catch (referralErr) {
        logger.warn('referral_conversion_skipped', { txId, reason: (referralErr as any)?.code || (referralErr as any)?.message });
      }
  ...
  static async issueRefund(
    ...
    try {
      ...
      // 7. Referral (Part 12): reverse attributed commission atomically with the refund.
      try {
        await ReferralService.reverseConversionByTransaction(String(transactionId), String(actorId || 'system'), reason);
      } catch (referralErr) {
        logger.warn('referral_reversal_skipped', { transactionId, reason: (referralErr as any)?.code || (referralErr as any)?.message });
      }
